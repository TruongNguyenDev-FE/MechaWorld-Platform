...
const ImageUpload = ({ primaryImage, setPrimaryImage, secondaryImages, setSecondaryImages }) => {
  ...
  const handleSecondaryUpload = ({ fileList }) => {
    ...
    if (secondaryImages.length + newFiles.length > 5) {
      return; // Không cho phép upload thêm ảnh
    }
    ...
    <div>
      ...
      <div className="mb-4">
        ...
        <Upload
          ...
          beforeUpload={() => false} onChange={handlePrimaryUpload}
          accept="image/*"
        >
      ...
      <div className="mt-6">
        <label className="font-bold flex items-center gap-1">
          ...
          <Tooltip title="Ảnh phụ để minh họa rõ hơn về sản phẩm từ nhiều góc độ khác nhau. Bạn chỉ có thể thêm tối đa 5 ảnh phụ.">
            <QuestionCircleOutlined className="text-gray-500 cursor-pointer" />
          </Tooltip>
        </label>
        ...
        <Upload
          ...
          maxCount={5}
        >
          {secondaryImages.length < 5 && "+ Thêm ảnh phụ"}
        </Upload>
      </div>
    </div>
  );
